...
function buildApiUrl({
  ...
  return `/api/Fran/fetchallbytype/${encodeURIComponent(type)}?${params.toString()}`;
}
...
export default function AllQueryClient({ initialQueries, initialPage, totalPages, users }) {
  ...
  const [hasMore, setHasMore] = useState(() => (totalPages ?? 1) > (initialPage ?? 1));
  const [isLoading, setIsLoading] = useState(false);
  const [isBootLoading, setIsBootLoading] = useState(false); // for filter reloads
  ...
  const [filterByGrade, setFilterByGrade] = useState("H"); // A/B/C
  ...
  const handleRowClick = (id) => router.push(`/main/page/fran/allquery/${id}`);
  ...
  // Map helpers
  const findUserNameById = useCallback(
    (id) => {
  ...
  const fetchFirstPageWithFilters = useCallback(async () => {
    ...
    try {
      ...
      // Update branches and admins from API response
      if (json?.branches) {
        setBranchOptions(json.branches);
      }
      if (json?.admins) {
  ...
  useEffect(() => {
    // only re-fetch on server-supported filters or when custom/range changes
    fetchFirstPageWithFilters();
  }, [fetchFirstPageWithFilters]);

  // ---- Infinite scroll loader ----
  const loadMore = useCallback(async () => {
    if (isLoading || isBootLoading || !hasMore) return;
    setIsLoading(true);
    const effectiveDeadlineFilter =
      deadlineFilter === "custom" && !customDate
        ? ""
  ...
  const handleExportAllQueries = useCallback(async () => {
    ...
    try {
      ...
      XLSX.writeFile(workbook, `queries-${today}.xlsx`);
    } catch (error) {
