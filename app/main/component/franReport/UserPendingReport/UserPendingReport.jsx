...
export default function UserPendingReport() {
  ...
  const fetchBranches = async () => {
    try {
      const res = await axios.get("/api/branch/fetchallserver/branch?franchise=true", {
        params: {
          limit: 88,
        }
      });
      setBranches(res.data.branches || res.data.branches || []);
    } catch {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get("/api/Fran/dailytaskget", { params });
