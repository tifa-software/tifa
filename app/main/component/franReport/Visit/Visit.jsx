...
export default function QueryReport() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const userResponse = await axios.get("/api/admin/fetchallserver/admin?franchisestaff=true", {
          params: {
            limit: 88,
          }
        });
        setuser(userResponse.data.users);
        const branchesResponse = await axios.get("/api/branch/fetchallserver/branch?franchise=true", {
          params: {
            limit: 88,
          },
        });
  ...
  const fetchFilteredData = async () => {
    setLoading(true);
    try {
      const response = await axios.get("/api/Fran/allvisit/query", {
        params: {
          referenceId,
          suboption,
          fromDate,
  ...
  const sortedQueries = useMemo(
    () => [...allquery].sort((a, b) => new Date(b.stage6UpdatedDate) - new Date(a.stage6UpdatedDate)),
    [allquery]
  );
          ...
          <table className="min-w-full text-left text-[12px] font-light border-collapse">
            <thead className="bg-gray-800 text-white">
              <tr className="divide-x divide-gray-700">
                ...
                <th className="px-4 py-3 text-[12px] relative group flex">Visit Date <ChevronDownSquare className=" ms-2" />
                ...
                sortedQueries.map((data, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-[12px]">
                      {(() => {
                        const date = new Date(data.stage6UpdatedDate);
                        const monthNames = [
