  ...
  useEffect(() => {
    ...
    useEffect(() => {
        const fetchuserData = async () => {
            if (adminData) {
            try {
                const response = await axios.get(`/api/admin/fetchall-bybranch/${adminData}`);
    ...
    const handleRowClick = (email) => {
        router.push(`/branch/page/user/${email}`);
    };
        ...
        <div className='container lg:w-[95%] mx-auto py-5'>
            ...
            <div className="flex justify-between items-center mb-4">
                ...
                <div className="hidden lg:flex space-x-3">
                    ...
                    <Link href={'/branch/page/registerstaff'}>
                        <button className="bg-[#29234b] rounded-md flex items-center text-white text-sm px-4 py-2 ">
                            <CirclePlus size={16} className='me-1' /> Add user
                        </button>
                    </Link>
                ...
                <table className="w-full text-sm text-left rtl:text-right text-gray-600 font-sans">
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            currentuser
                                .filter(user => user.usertype !== "1") // Filter out users with usertype "1"
                                .map((user, index) => (
                                    <tr
                                        key={user._id}
